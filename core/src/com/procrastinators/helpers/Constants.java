package com.procrastinators.helpers;

/**
 * Created by jazeem on 24/05/17.
 */

public class Constants {
    public static final int GAME_WIDTH = 1280;
    public static final int GAME_HEIGHT = 720;
    public static final int CARD_SEPERATION = 15;
    public static final int USER_CARD_SEPERATION = 60;
    public static final int CARD_WIDTH = 73;
    public static final int USER_CARD_WIDTH = 110;
    public static final int CARD_HEIGHT = 98;
    public static final int USER_CARD_HEIGHT = 147;
    public static final int OPPONENT_CARD_GAP = 50;
    public static final float TURN_TIME = (float) 2;
    public static final int SPADE_ACE_CODE = 13;
    public static final boolean FLIP_OPPONENT_CARDS = true;
}
